...
game=rules.create(['Solo'],cards,()=>.2);assert.equal(game.players.length,1);for(let i=0;i<3;i++){const wrong=(correctSlot(game)+1)%(game.players[0].lane.length+1);rules.guess(game,wrong,cards);assert.equal(game.result.kind,'discard');assert.equal(game.mistakes,i+1);rules.next(game);}assert.equal(game.phase,'end');assert.equal(game.winners.length,0);
...
console.log('PASS: 200 unique half-point cards, solo win/three-strike loss, passing without reveal, discard, stealing, wins for 2/4/8 players, deck exhaustion, Orbit single RAF over 7200 frames, bounded bullets, shield loss and replay.');
